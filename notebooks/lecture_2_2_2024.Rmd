...
Check this: This is approximately the 95% confidence interval:

```{r}
1.96051 - 1.96 * 0.50312 # lower bound on β0
```

```{r}
1.96051 + 1.96 * 0.50312 # upper bound on β0
```

These are fairly broad for the true value of $\beta_0$ and $\beta_1$.

```{r}
confint(lm1,level=0.99) # 99% CI for the coefficients
```
...
Each row corresponds to the $x_i$ in the `df` , giving the estimated $\hat{\beta_0} + \hat{\beta_1}x_i$ and the upper and lower prediction intervals (defaulting to 95%).
